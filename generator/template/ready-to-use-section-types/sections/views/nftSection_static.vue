<template>
  <div v-if="show" id="nftSection" class="rowStyle items-center my-8">
    <div class="text-center blockStyle">
      <div>
        <vue-metamask ref="metamaskStatic" :init-connect="false" @onComplete="onComplete"></vue-metamask>
      ...
      <div class="flex flex-row align-between items-center p-4">
        ...
        <div class="flex flex-row items-center">
          <div class="font-bold" :class="(amount <= 0) || (isPublicSaleActive === false && !isWhitelist) ? 'fontStyleDisabled' : 'fontStyle'" :style="(amount <= 0) || (isPublicSaleActive === false && !isWhitelist) ? 'color: Silver; cursor: default' : 'color: white; cursor: pointer'" @click="amount > 0 ? amount = amount - 1 : {}">-</div>
<!--          <input v-model="amount" class="bg-transparent text-textSubheading amountInputStyle" type="number"/>-->
          ...
          <div :class="(amount >= Math.min(maxBuyPerAddress - mintsCount, maxSupply - totalSupply)) || (isPublicSaleActive === false && !isWhitelist) ? 'fontStyleDisabled' : 'fontStyle'" :style="(amount >= Math.min(maxBuyPerAddress - mintsCount, maxSupply - totalSupply)) || (isPublicSaleActive === false && !isWhitelist) ? 'color: Silver; cursor: default' : 'color: white; cursor: pointer'" @click="amount < Math.min(maxBuyPerAddress - mintsCount, maxSupply - totalSupply) ? amount = amount + 1 : {}">+</div>
      ...
      <div>
        <button v-if="connected && (parseInt(mintsCount) < parseInt(maxBuyPerAddress)) && (parseInt(totalSupply) < parseInt(maxSupply)) && (isPublicSaleActive === true || isWhitelist) && isContractPaused === false" class="h-53px py-3 md:px-8 mr-4 px-4 m-1 text-sm rounded-full" :class="submitButtonStyle" :style="processing === true ? 'background-color: Silver' : 'background-color: black'" @click="callMintFunction">
        ...
        <button v-else-if="connected && ((isPublicSaleActive === false  && !isWhitelist) || isContractPaused === true)" class="h-53px py-3 md:px-8 mr-4 px-4 m-1 text-sm text-textSubheading rounded-full" :class="submitButtonStyle" @click="() => {}">
          Unavailable
        </button>
        ...
        <p v-if="connected && isPublicSaleActive === false  && !isWhitelist" class="italic text-textSubheading w-400px">Public mint is not open, come back later</p>
...
export default {
  ...
  methods: {
    initMetamask() {
      if(window.web3) {
        if(this.$refs.metamaskStatic) {
          this.$refs.metamaskStatic.init()
        }
      } else {
        ...
        try {
          ...
          if(this.tokenType === 'Erc1155') {
            ...
            nftContract.methods.initFront(this.web3.coinbase, this.section.settings[0].token_id).call().then((res) => {
              ...
              this.isPublicSaleActive = res.active
    ...
    async callMintFunction() {
      ...
      const contractAddress = this.contractAddr
      const nftContract = new this.$refs.metamaskStatic.web3.eth.Contract(JSON.parse(this.contractABI), contractAddress)
      const nonce = await this.$refs.metamaskStatic.web3.eth.getTransactionCount(this.web3.coinbase, 'latest') // get latest nonce
